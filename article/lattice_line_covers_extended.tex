...
Figure~\ref{fig:directions}, not as discrete points but as an accumulating measure on the closed
unit disc: each realized direction contributes a weighted point mass at its two antipodal boundary
points and a line mass along its diameter chord, with weight \(1/(k+5)\) so early directions
dominate but not overwhelmingly. Nothing in this manuscript rests on it --- see the Acknowledgements
for how it came about.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/disc_measure.png}
  \caption{The same \(100\) realized directions as Figure~\ref{fig:directions}, redrawn as an
  accumulating measure on the closed unit disc rather than as discrete points: each direction
  contributes a weighted point mass at its two boundary points plus a line mass along its diameter
  chord, weight \(1/(k+5)\), shown at the same growing prefix lengths (here \(10\), \(20\), \(50\),
  \(100\)). Purely exploratory --- not used anywhere else in this manuscript.}
