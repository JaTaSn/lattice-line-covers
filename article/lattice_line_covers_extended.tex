...
\begin{remark}[The two selection rules]
  ...
  constraint of no use here --- the possible order types are exactly \(\omega^1,\dots,\omega^n\); see
  Robbiano's classification~\cite{Robbiano1985}. The moral survives in the present looser setting:
  what a terminating search needs is not compatibility with the group structure, but order type
  \(\omega\).)
