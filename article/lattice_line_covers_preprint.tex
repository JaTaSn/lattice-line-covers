...
\title{On the directions occurring in lattice-line coverings of the integer plane}
...
\begin{remark}[The lattice-line hypothesis]
  ...
  \begin{equation}
    ...
    L_z\cap\Z^2=\{z\}
  \end{equation}
  (a second lattice point on \(L_z\) would force \(\alpha_z\in\mathbb Q\)), so distinct such lines never
  ...
  interest in this note therefore concerns the restriction to \emph{lattice lines}: lines
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[scale=0.9]
    ...
    \foreach \x in {-1,...,4} \foreach \y in {-1,...,4} \fill[black] (\x,\y) circle (1.1pt);
...
\begin{example}
  ...
  \eqref{eq:splitting-coset-conclusion}:
  \begin{align}
    (u,w)=(1,2) &\;\longmapsto\; (x,y)=(3,7): && \varphi_{(4,9)}(3,7)=27-28=-1=c_0+36\cdot0, \\
    (u,w)=(3,2) &\;\longmapsto\; (x,y)=(7,7): && \varphi_{(4,9)}(7,7)=63-28=35=c_0+36\cdot1, \\
    (u,w)=(1,5) &\;\longmapsto\; (x,y)=(3,16): && \varphi_{(4,9)}(3,16)=27-64=-37=c_0+36\cdot(-1),
  \end{align}
...
\begin{proof}[Proof of Theorem~\ref{thm:main}]
  Fix an enumeration \((z_k)_{k\ge1}\) of \(\Z^2\) and a sequence \((\theta_k)_{k\ge1}\) dense in
  ...
  need be avoided); otherwise reserve the lexicographically-least \((u_0,w_0)\) outright. A concrete
  rule is needed for the recursion to be a well-defined function, as is a concrete choice among the
  \((s,t)\) that Lemma~\ref{lem:steering} only asserts to exist. Both are choice functions to be
  supplied explicitly in a Lean formalization, not just existence claims. Let
  \(R_k=R_{u_0,w_0}\) be that reserved sub-coset.
  ...
  Lemma~\ref{lem:splitting-coset}). So stage \(k\)'s claimed points lie in \(R_{k-1}\setminus
  R_k\subset R_{k-1}\). For \(j>k\), \(R_{j-1}\subseteq R_k\) (as \(R_i\subseteq R_{i-1}\) for every \(i\)),
  ...
  either. (Different stages in fact always realize different directions: \(|P_k|=n_1^{(k)}\) is
  strictly increasing in \(k\), since \(n_1^{(k)}=n_1^{(k-1)}|s_k|\ge2n_1^{(k-1)}\); though the
...
\section{A picture of the construction}

Figure~\ref{fig:directions} plots the first \(10\), \(20\), and \(100\) directions realized by an actual
run of the construction (code in \texttt{code/}, targeting the golden-angle equidistributed
sequence \(\theta_k = k\pi(\sqrt5-1)\bmod\pi\)), each direction shown as an antipodal pair of points
...
\begin{remark}
  ...
  Claude agent) working from the statement of the lemmas alone, who found a real gap in an earlier
  version of the density argument (restricting \(s,t\) to be positive only reaches half of \(\RP\),
  and existence of \emph{some} admissible move at each stage does not imply the ability to steer
...
is the natural two-dimensional analogue of the classical theory of \emph{covering systems of
...
The forbidden unimodular pairs of Lemma~\ref{lem:rigidity} are exactly the Farey-neighbor pairs of
...
This problem occurred to the author during an idle conversation with colleagues (in the department
coffee room) about covering the lattice \(\Z^2\) with lines. It was considered as a useful test for
LLM-assisted proof discovery and proof formalization, and indeed the results in this note --- the
reformulation, the rigidity and coset lemmas, the recursive construction, and both rounds of
error-correction (one caught by numerical simulation, one by an independent review) --- were found
by Claude (Anthropic), working with the author in an interactive session. The additional
references collected in the extended-references version of this note were located and verified by
Claude via web search.\footnote{See~\cite{gitrepo} for a list of ``certificates'' showing how each
reference is used in this manuscript.} This manuscript was likewise produced by the author and
Claude jointly.
